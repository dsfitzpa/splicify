...
export async function POST(req: Request) {
  ...
  // Derive the plannotate endpoint from the backend base URL
  // e.g. https://api.example.com/api/chat  →  https://api.example.com/plannotate/annotate_sequence
  ...
  const candidates = [
    {
      url: `${backendBase}/plannotate/annotate_sequence_with_hierarchy`,
      proxyTarget: "annotate_sequence_with_hierarchy",
      backendRef: backendBase,
    },
    {
      url: `${backendOrigin}/plannotate/annotate_sequence_with_hierarchy`,
      proxyTarget: "annotate_sequence_with_hierarchy",
      backendRef: backendOrigin,
    },
    {
      url: `${backendBase}/plannotate/annotate_sequence`,
      proxyTarget: "annotate_sequence",
      backendRef: backendBase,
    },
    {
      url: `${backendOrigin}/plannotate/annotate_sequence`,
      proxyTarget: "annotate_sequence",
      backendRef: backendOrigin,
    },
  ];
  ...
  for (const candidate of candidates) {
    ...
    const isNotFoundLike =
      ...
      (upstreamRes.status === 422 && !data?.ok) ||
    ...
    if (!upstreamRes.ok || data?.ok === false) {
      return NextResponse.json(
        {
          ok: false,
