...
function extractErrorMessage(data: any): string {
  ...
  return "Primer-design request failed";
}
...
export async function POST(req: Request) {
  ...
  // Use the /plannotate/design_primers alias rather than the bare
  // /design-primers route. The alias is delegated by plannotate_router to
  // pcr.design_primers, which keeps the call inside the /plannotate/* path
  // prefix that reverse proxies in front of the FastAPI are known to allow
  // (the pcr router itself is mounted at root, which some setups do not
  // expose externally).
  const candidates = [
    {
      url: `${backendBase}/plannotate/design_primers`,
      proxyTarget: "design_primers",
      backendRef: backendBase,
    },
    {
      url: `${backendOrigin}/plannotate/design_primers`,
      proxyTarget: "design_primers",
